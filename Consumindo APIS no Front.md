...
export default function App() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 1) Requisição HTTP GET com fetch (nativo do navegador)
      const response = await fetch(url, {
        method: 'GET',
        headers: { Accept: 'application/json' },
      })

      // 2) Checa status HTTP (200–299)
      if (!response.ok) {
        throw new Error(`Erro HTTP: ${response.status}`)
      }

      // 3) Converte o corpo para objeto JS
      const data = await response.json()

      // 4) Regra da API: CEP inexistente vem com { erro: true }
      if (data.erro) {
      ...
      console.error(err)
      setError('Não foi possível buscar o CEP. Verifique sua conexão e tente novamente.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="container">
      ...
      <p className="subtitle">Digite um CEP e descubra o endereço completo.</p>
